...
function Btn({onClick,disabled,children,variant="primary",size="md"}:{onClick?:()=>void;disabled?:boolean;children:React.ReactNode;variant?:"primary"|"secondary"|"ghost";size?:"sm"|"md"}) {
  ...
  const variants={
    primary:"bg-terracotta text-white hover:bg-terracotta-600",
    secondary:"border border-charcoal/10 bg-white text-charcoal hover:border-terracotta/40 hover:text-terracotta-600",
    ghost:"text-charcoal/55 hover:text-charcoal bg-transparent border-0",
  };
  return <button onClick={onClick} disabled={disabled} className={`${base} ${sizes[size]} ${variants[variant]}`}>{children}</button>;
}

function Card({children,className=""}:{children:React.ReactNode;className?:string}) {
  return <div className={`rounded-2xl border border-charcoal/10 bg-white p-5 shadow-sm ${className}`}>{children}</div>;
}

function InfoBox({children}:{children:React.ReactNode}) {
  return <div className="rounded-2xl bg-dustyblue-50 border border-dustyblue-800/10 px-5 py-4 text-sm text-dustyblue-800 leading-7">{children}</div>;
}

function ResultBox({children}:{children:React.ReactNode}) {
  return <Card className="text-sm leading-7 text-charcoal/75 whitespace-pre-wrap">{children}</Card>;
}

function ChatBubble({role,content}:{role:string;content:string}) {
  return (
    <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 whitespace-pre-wrap ${role==="user"?"self-end bg-dustyblue-50 border border-dustyblue-800/10 text-dustyblue-800":"self-start bg-white border border-charcoal/10 text-charcoal/75"}`}>
      {content}
    </div>
  );
}
...
function FileAttach({onFile,fileName,onClear}:{onFile:(content:string,name:string)=>void;fileName:string|null;onClear:()=>void}) {
  ...
  return (
    <div onClick={()=>ref.current?.click()} className={`rounded-2xl border-2 border-dashed px-4 py-3 cursor-pointer transition flex items-center gap-3 ${fileName?"border-terracotta/40 bg-terracotta/5":"border-charcoal/15 bg-white hover:border-terracotta/40 hover:bg-terracotta/5"}`}>
      <i className="ti ti-paperclip text-base text-terracotta shrink-0"></i>
      <div className="flex-1 min-w-0">
        {fileName?(
          <div className="flex items-center justify-between gap-2">
            <span className="text-sm font-medium text-terracotta-600 truncate">{fileName}</span>
            <button onClick={e=>{e.stopPropagation();onClear();}} className="text-xs text-charcoal/40 hover:text-terracotta shrink-0">✕</button>
          </div>
        ):(
          <div>
            <p className="text-sm font-medium text-charcoal">Attach a data file <span className="text-charcoal/40 font-normal">(optional)</span></p>
            <p className="text-xs text-charcoal/40 mt-0.5">CSV, TXT, JSON, MD — attach and skip the questions</p>
          </div>
        )}
      </div>
...
function PreflightForm({moduleId,onSubmit}:{moduleId:string;onSubmit:(reply:string,ctx:string)=>void}) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex rounded-2xl border border-charcoal/10 bg-white p-1 gap-1 w-fit shadow-sm">
        <button onClick={()=>setMode("questions")} className={`px-4 py-2 text-sm rounded-xl font-medium transition ${mode==="questions"?"bg-terracotta text-white":"text-charcoal/55 hover:text-charcoal"}`}>Guided Questions</button>
        <button onClick={()=>setMode("custom")} className={`px-4 py-2 text-sm rounded-xl font-medium transition ${mode==="custom"?"bg-terracotta text-white":"text-charcoal/55 hover:text-charcoal"}`}>Custom Prompt</button>
      </div>
      ...
      {mode==="questions"&&(
        <>
          ...
          {config.questions.map((q,i)=>(
            <Card key={q.key}>
              <div className="text-sm font-medium text-charcoal mb-2"><span className="text-charcoal/40 mr-2">{i+1}</span>{q.label}</div>
              <textarea rows={q.rows||2} value={answers[q.key]||""} onChange={e=>setAnswers({...answers,[q.key]:e.target.value})} placeholder={q.placeholder}
                className="w-full text-sm resize-y rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50 focus:bg-white transition"/>
            </Card>
          ))}
          <div className="flex items-center gap-3 flex-wrap">
            <Btn onClick={handle} disabled={loading||!canSubmit}>{loading?"Generating...":"Generate Analysis →"}</Btn>
            <span className="text-xs text-charcoal/40">{filled}/{config.questions.length} answered{fileName?" · file attached":""}</span>
          </div>
        </>
      )}
      {mode==="custom"&&(
        <>
          <Card>
            <div className="text-sm font-medium text-charcoal mb-2">Write your custom prompt</div>
            <textarea rows={6} value={customPrompt} onChange={e=>setCustomPrompt(e.target.value)}
              ...
              className="w-full text-sm resize-y rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50 focus:bg-white transition"/>
            <p className="text-xs text-charcoal/40 mt-2">The AI will still use the {config.title} specialist context.</p>
          </Card>
      ...
      {phase==="chat"&&(
        <div className="flex flex-col h-[420px] gap-3">
          <div className="flex-1 overflow-y-auto flex flex-col gap-3 p-1">
            <div className="self-start max-w-[85%] bg-white border border-charcoal/10 rounded-2xl px-4 py-3 text-sm leading-7 text-charcoal/75 whitespace-pre-wrap">{result}</div>
            {chatMsgs.map((m,i)=><ChatBubble key={i} role={m.role} content={m.content}/>)}
            {chatLoading&&<div className="self-start bg-white border border-charcoal/10 rounded-2xl px-4 py-3 text-sm text-charcoal/40">Thinking...</div>}
            <div ref={bottomRef}/>
          </div>
          <div className="flex gap-2">
            <textarea value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChat();}}} placeholder="Ask a follow-up... (Enter to send)" rows={2}
              className="flex-1 resize-none text-sm rounded-xl border border-charcoal/10 bg-white px-3 py-2 outline-none focus:border-terracotta/50 transition"/>
...
function FunnelModule() {
  ...
  if (phase==="setup"||phase==="data") return (
    <div className="flex flex-col gap-4">
      <div className="flex gap-2 flex-wrap">
        {[["saas","SaaS"],["ecommerce","E-commerce"],["onboarding","Onboarding"],["custom","Custom"]].map(([val,label])=>(
          <button key={val} onClick={()=>applyTemplate(val)}
            className={`px-4 py-2 text-sm rounded-xl border transition ${template===val?"border-terracotta/50 bg-terracotta/5 text-terracotta-600 font-semibold":"border-charcoal/10 bg-white text-charcoal/70 hover:border-terracotta/30"}`}>{label}</button>
        ))}
      </div>
      ...
      {fileContent ? (
        ...
        <div className="flex flex-col gap-3">
          {steps.map((s,i)=>{
            ...
            const barColor=i===0?"bg-dustyblue-400":dr&&+dr>50?"bg-terracotta":dr&&+dr>25?"bg-sand-500":"bg-sage-500";
            return (
              <Card key={i} className="p-4">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-xs text-charcoal/40 w-5 shrink-0">{i+1}</span>
                  <input value={s.name} onChange={e=>upd(i,"name",e.target.value)}
                    className="flex-1 font-semibold text-sm text-charcoal border-b border-dashed border-charcoal/15 bg-transparent outline-none pb-1"/>
                  {dr&&<span className={`text-xs font-semibold shrink-0 ${+dr>40?"text-terracotta":"text-sand-800"}`}>-{dr}%</span>}
                  <button onClick={()=>setSteps(steps.filter((_,j)=>j!==i))} disabled={steps.length<=2} className="text-xs text-terracotta/60 hover:text-terracotta disabled:opacity-30 shrink-0">✕</button>
                </div>
                <input type="number" value={s.users||""} onChange={e=>upd(i,"users",+e.target.value)} placeholder="Enter number of users at this step"
                  className="w-full text-sm rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50"/>
                {s.users>0&&<div className="mt-2 h-1.5 bg-charcoal/5 rounded-full"><div className={`h-full ${barColor} rounded-full transition-all`} style={{width:`${pct}%`}}/></div>}
              </Card>
            );
          })}
          <div className="flex gap-2">
            <input value={newStep} onChange={e=>setNewStep(e.target.value)} onKeyDown={e=>e.key==="Enter"&&newStep.trim()&&(setSteps([...steps,{name:newStep,users:0}]),setNewStep(""))}
              placeholder="Add a step..." className="flex-1 text-sm rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50"/>
      ...
      <Card>
        <div className="text-sm font-medium text-charcoal mb-2">Custom prompt <span className="text-charcoal/40 font-normal text-xs">(optional)</span></div>
        <textarea rows={2} value={customPrompt} onChange={e=>setCustomPrompt(e.target.value)}
          ...
          className="w-full text-sm resize-y rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50"/>
      </Card>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {steps.map((s,i)=>{
          ...
          return (
            <Card key={i} className="p-4">
              <div className="text-xs text-charcoal/40 mb-1">{s.name}</div>
              <div className="text-xl font-bold text-charcoal">{s.users.toLocaleString()}</div>
              <div className="text-xs text-charcoal/40">{cr}% overall</div>
              {dr&&<div className={`text-xs font-semibold ${+dr>40?"text-terracotta":"text-sand-800"}`}>-{dr}%</div>}
            </Card>
          );
        })}
      </div>
      ...
      <div className="flex gap-2">
        <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendChat()} placeholder="Ask a follow-up..."
          className="flex-1 text-sm rounded-xl border border-charcoal/10 bg-white px-3 py-2 outline-none focus:border-terracotta/50"/>
...
function HypothesisModule() {
  ...
  const cc:Record<string,string>={low:"#C1694F",medium:"#B89A6E",high:"#5C8567"};
  ...
  return (
    <div className="flex flex-col gap-4">
      {show&&(
        <Card>
          <div className="text-sm font-semibold text-charcoal mb-3">Build a new hypothesis</div>
          {[{k:"if",l:"If...",p:"we add feature X"},{k:"then",l:"Then...",p:"conversion will increase"},{k:"because",l:"Because...",p:"users struggle with Z"},{k:"metric",l:"Success metric",p:"e.g. 10% lift"},{k:"timeline",l:"Timeline",p:"e.g. 2-week A/B test"},{k:"evidence",l:"Evidence",p:"optional"}].map(f=>(
            <div key={f.k} className="flex gap-3 items-center mb-2 flex-wrap">
              <span className="text-xs text-charcoal/40 w-20 shrink-0">{f.l}</span>
              <input value={(form as Record<string,string>)[f.k]} onChange={e=>setForm({...form,[f.k]:e.target.value})} placeholder={f.p}
                className="flex-1 min-w-[160px] text-sm rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50"/>
            </div>
          ))}
          <div className="flex gap-3 items-center mt-2 flex-wrap">
            <span className="text-xs text-charcoal/40 w-20 shrink-0">Confidence</span>
            <select value={form.confidence} onChange={e=>setForm({...form,confidence:e.target.value})}
              className="text-sm rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none">
      ...
      {hyps.map(h=>(
        <Card key={h.id}>
          <div className="flex justify-between gap-3 mb-2 flex-wrap">
            <p className="text-sm leading-7 text-charcoal/75"><span className="text-charcoal/40">If</span> {h.if} <span className="text-charcoal/40">then</span> {h.then}{h.because&&<><span className="text-charcoal/40"> because</span> {h.because}</>}</p>
          ...
          {h.metric&&<p className="text-xs text-charcoal/40 mb-3">Metric: {h.metric} · Timeline: {h.timeline}</p>}
          <Btn variant="secondary" size="sm" onClick={()=>validate(h)} disabled={loading===h.id}>{loading===h.id?"Validating...":"AI Validate →"}</Btn>
          {aiResult[h.id]&&<div className="mt-3 text-sm leading-7 text-charcoal/70 whitespace-pre-wrap border-t border-charcoal/10 pt-3">{aiResult[h.id]}</div>}
        </Card>
      ))}
      {hyps.length===0&&!show&&<p className="text-sm text-charcoal/40 text-center py-8">No hypotheses yet.</p>}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <select value={fw} onChange={e=>setFw(e.target.value)} className="text-sm rounded-xl border border-charcoal/10 bg-white px-3 py-2 outline-none">
      ...
      <div className="overflow-x-auto rounded-2xl border border-charcoal/10 bg-white shadow-sm">
        <table className="w-full min-w-[520px] text-sm border-collapse">
          <thead><tr className="border-b border-charcoal/5">{["Feature","Reach","Impact","Conf%","Effort","RICE","ICE"].map(h=>(
            <th key={h} className="px-3 py-3 text-left text-xs font-semibold text-charcoal/40 uppercase tracking-wide">{h}</th>
          ))}</tr></thead>
          <tbody>{sorted.map((f,i)=>(
            <tr key={i} className="border-b border-charcoal/5 last:border-0">
              <td className="px-3 py-3 font-semibold text-charcoal text-xs">{f.name}</td>
              {["reach","impact","confidence","effort"].map(k=>(
                <td key={k} className="px-3 py-2">
                  <input type="number" value={(f as unknown as Record<string,number>)[k]} min={0} max={k==="confidence"?100:10}
                    ...
                    className="w-12 text-sm rounded-lg border border-charcoal/10 bg-cream px-2 py-1 outline-none focus:border-terracotta/50"/>
                </td>
              ))}
              <td className={`px-3 py-3 font-bold text-sm ${fw==="rice"?"text-terracotta-600":"text-charcoal/30"}`}>{rice(f)}</td>
              <td className={`px-3 py-3 font-bold text-sm ${fw==="ice"?"text-terracotta-600":"text-charcoal/30"}`}>{ice(f)}</td>
            </tr>
          ))}</tbody>
        </table>
      </div>
...
function StakeholderModule() {
  ...
  if (phase==="pick-tool") return (
    <div className="flex flex-col gap-4">
      <p className="text-sm font-semibold text-charcoal">What do you want to draft?</p>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {PM_TOOLS.map(t=>(
          <button key={t.id} onClick={()=>{setTool(t.id);setPhase("pick-tone");}}
            className="p-4 rounded-2xl border border-charcoal/10 bg-white text-left hover:border-terracotta/40 hover:bg-terracotta/5 transition shadow-sm">
            <i className={`${t.icon} text-xl text-terracotta mb-2 block`}></i>
            <div className="text-sm font-semibold text-charcoal">{t.label}</div>
            <div className="text-xs text-charcoal/55 mt-1 leading-4">{t.desc}</div>
          </button>
        ))}
      </div>
    </div>
  ...
  if (phase==="pick-tone") return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-sm font-semibold text-charcoal">Choose a tone</p>
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {TONES.map(t=>(
          <button key={t} onClick={()=>{setTone(t);setPhase("form");}}
            className="p-3 rounded-2xl border border-charcoal/10 bg-white text-left hover:border-terracotta/40 hover:bg-terracotta/5 transition">
            <div className="text-sm font-semibold text-charcoal mb-1">{t}</div>
            <div className="text-xs text-charcoal/55 leading-4">{TONE_DESC[t]}</div>
          </button>
        ))}
      </div>
    </div>
    ...
    return (
      <div className="flex flex-col gap-4">
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <span className="text-xs bg-dustyblue-50 text-dustyblue-800 border border-dustyblue-800/10 rounded-full px-3 py-1">{tone}</span>
          <span className="text-xs bg-cream text-charcoal/70 border border-charcoal/10 rounded-full px-3 py-1 flex items-center gap-1">
        ...
        {qs.map(q=>(
          <Card key={q.key}>
            <div className="text-sm font-medium text-charcoal mb-2">{q.label}</div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl border border-charcoal/10 bg-white p-5 text-sm leading-7 text-charcoal/75 whitespace-pre-wrap">{result}</div>
      <div className="text-xs font-bold uppercase tracking-widest text-terracotta">Revise</div>
      <div className="flex flex-col gap-3 max-h-60 overflow-y-auto">
        {chatMsgs.map((m,i)=><ChatBubble key={i} role={m.role} content={m.content}/>)}
        {chatLoading&&<div className="self-start bg-white border border-charcoal/10 rounded-2xl px-4 py-3 text-sm text-charcoal/40">Revising...</div>}
        <div ref={bottomRef}/>
      </div>
      <div className="flex gap-2">
...
function RepositoryModule() {
  ...
  return (
    <div className="flex flex-col gap-4">
      {show&&(
        <Card>
          <div className="text-sm font-semibold text-charcoal mb-3">Add new insight</div>
          {[{k:"title",l:"Title",p:"e.g. Users abandon checkout on mobile"},{k:"source",l:"Source",p:"e.g. User interview"},{k:"date",l:"Date",p:"e.g. May 2025"},{k:"tags",l:"Tags",p:"e.g. mobile, checkout"}].map(f=>(
            <div key={f.k} className="flex gap-3 items-center mb-2 flex-wrap">
              <span className="text-xs text-charcoal/40 w-14 shrink-0">{f.l}</span>
              <input value={(form as Record<string,string>)[f.k]} onChange={e=>setForm({...form,[f.k]:e.target.value})} placeholder={f.p}
                className="flex-1 min-w-[160px] text-sm rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50"/>
            </div>
          ))}
          <div className="flex gap-3 items-start mb-3">
            <span className="text-xs text-charcoal/40 w-14 shrink-0 pt-2">Insight</span>
            <textarea rows={3} value={form.summary} onChange={e=>setForm({...form,summary:e.target.value})} placeholder="Describe the insight..."
              className="flex-1 text-sm rounded-xl border border-charcoal/10 bg-cream px-3 py-2 outline-none focus:border-terracotta/50 resize-none"/>
          </div>
      ...
      {filtered.length===0&&<p className="text-sm text-charcoal/40 text-center py-8">No insights yet.</p>}
      {filtered.map(i=>(
        <Card key={i.id}>
          <div className="flex justify-between items-start gap-3 mb-2">
            <div className="text-sm font-semibold text-charcoal">{i.title}</div>
            <button onClick={()=>setInsights(prev=>prev.filter(x=>x.id!==i.id))} className="text-xs text-charcoal/40 hover:text-terracotta shrink-0">✕</button>
          </div>
          <div className="flex gap-2 flex-wrap mb-2">
            {i.source&&<span className="text-xs bg-cream text-charcoal/60 px-2 py-0.5 rounded-lg">{i.source}</span>}
            {i.date&&<span className="text-xs bg-cream text-charcoal/60 px-2 py-0.5 rounded-lg">{i.date}</span>}
            {i.tags.split(",").filter(Boolean).map(t=>(
              <span key={t} className="text-xs bg-dustyblue-50 text-dustyblue-800 px-2 py-0.5 rounded-lg">{t.trim()}</span>
            ))}
          </div>
          {i.aiSummary&&<div className="text-xs text-charcoal/65 leading-5 bg-cream rounded-xl p-3 whitespace-pre-wrap">{i.aiSummary}</div>}
        </Card>
      ))}
    </div>
  );
...
function Dashboard({onNavigate}:{onNavigate:(id:string)=>void}) {
  ...
  const featured=[
    {id:"funnel",tone:"bg-sage-600"},
    {id:"feature-research",tone:"bg-dustyblue-600"},
    {id:"prd",tone:"bg-sand-600"},
    {id:"hypothesis",tone:"bg-terracotta"},
    {id:"brainstorm",tone:"bg-sage-600"},
    {id:"stakeholder",tone:"bg-dustyblue-600"},
  ];
  return (
    <div className="flex flex-col gap-6">
          ...
          {featured.map(f=>{
            ...
            return mod?(
              <button key={f.id} onClick={()=>onNavigate(f.id)}
                className={`text-left flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-2xl ${f.tone} text-white transition hover:scale-[1.02]`}>
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-terracotta mb-3">All Modules</p>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {tools.map(m=>(
            <button key={m.id} onClick={()=>onNavigate(m.id)}
              className="text-left flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-2xl border border-charcoal/10 bg-white text-charcoal/75 shadow-sm transition hover:-translate-y-0.5 hover:border-terracotta/30 hover:text-terracotta-600 hover:shadow-md">
              <i className={`${m.icon} text-base text-terracotta shrink-0`}></i>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-cream">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Desktop Sidebar */}
        <div className="hidden md:flex w-56 shrink-0 flex-col border-r border-charcoal/10 bg-white">
          <div className="flex-1 overflow-y-auto py-2">
            {MODULES.map(m=>(
              <button key={m.id} onClick={()=>navigate(m.id)}
                className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition border-l-2 justify-start ${active===m.id?"bg-sage-50 text-sage-800 border-terracotta font-semibold":"text-charcoal/60 hover:bg-cream hover:text-charcoal border-transparent"}`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="px-4 py-3 bg-white border-b border-charcoal/10 flex items-center gap-3 shrink-0">
            <button onClick={()=>setMobileMenuOpen(true)} className="md:hidden p-1.5 rounded-lg text-charcoal/60 hover:bg-cream transition shrink-0" aria-label="Open module menu">
              <i className="ti ti-menu-2 text-lg"></i>
            </button>
            <i className={`${mod?.icon} text-lg hidden md:block text-terracotta`}></i>
            <span className="text-sm font-bold text-charcoal truncate">{mod?.label}</span>
          </div>
        ...
        <div className="fixed inset-0 z-[70] md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 max-w-[80vw] bg-white shadow-xl flex flex-col">
            <div className="px-4 py-4 border-b border-charcoal/10 flex items-center justify-between">
              <span className="text-sm font-bold text-charcoal">PM Hub modules</span>
              <button onClick={()=>setMobileMenuOpen(false)} className="text-charcoal/50 hover:text-charcoal p-1 rounded-lg" aria-label="Close menu">
            ...
            <div className="flex-1 overflow-y-auto py-2">
              {MODULES.map(m=>(
                <button key={m.id} onClick={()=>navigate(m.id)}
                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition border-l-2 justify-start ${active===m.id?"bg-sage-50 text-sage-800 border-terracotta font-semibold":"text-charcoal/60 hover:bg-cream border-transparent"}`}>
                  <i className={`${m.icon} text-base shrink-0`}></i>
                  <span className="truncate">{m.label}</span>
                </button>
              ))}
